...
const getPokemon = async function (start, stop) {
  ...
  for (let i = 0; i < div.childNodes.length; i++) {
    idArray.push(div.childNodes[i].dataset.pokeNumber);
  }
  ...
  if (idArray.length !== 0) {
    ...
    for (let i = start; i < stop; i++) {
        ...
        const pokemonCard = new aPokemonCard(
          pokemon.id,
          pokemon.name,
          pokemon.sprites.front_default,
          pokemon.types
        );
        allPokemon.push(pokemonCard);
        // renderPokemon(pokemonCard)
      }
      console.log(idArray)
      let filteredPokemon = allPokemon.filter((pokemon) =>
        {!idArray.some(pokemon.id)
    ...
    // allPokemon.filter(thingie => thingie.id === )
...
function renderPokemon(pokemonData) {
  ...
  pokemonData.forEach((pokemonCard) => {
    ...
    for (let x = 0; x < pokemonCard.type.length; x++)
      pokemonTypes.innerText =
        pokemonTypes.innerText + pokemonCard.type[x] + ` `;
...
const favoritesRender = (e, pokemonCard) => {
  ...
  console.log(e.target.parentNode.dataset.pokemonNumber);
  // let unfavoriteItem = e.target.parentNode.dataset.pokemonNumber
  // let matchingItem = document.querySelector(`[data-pokemon-number~="${unfavoriteItem}"]`)
  // console.log(matchingItem)
  // if (e.target.parentNode.dataset.pokemonNumber == matchingItem)
  ...
  const favoritesText = document.createElement(`p`);
  const likeContainerChildren = document.querySelector(`#likeContainer`)
    .childNodes;
...
//   for (let i = 0; i < likeContainerChildren.length; i++) {
//     console.log(e.target.parentNode.dataset.pokemonNumber);
//     idArray.push(likeContainerChildren[i].dataset.pokeNumber);
    // if (likeContainerChildren[i].dataset.pokeNumber == e.target.parentNode.dataset.pokemonNumber) {
    //     let unfavoriteItem = e.target.parentNode.dataset.pokemonNumber
    //     let matchingItem = document.querySelector(`[data-pokemon-number~="${unfavoriteItem}"]`)
    //     matchingItem.className = 'hidden'
    //     console.log(`success`)
    // }
//   }

//   let filteredPokemon = allPokemon.filter((pokemon) =>
//     idArray.includes(pokemon.id)
//   );
//   renderPokemon(filteredPokemon);

  // for (let i=0; i < likeContainerChildren.length; i++) {
  //     console.log(e.target.parentNode.dataset.pokemonNumber)
  //     console.log(likeContainerChildren[0].dataset.pokeNumber)
  //     if (likeContainerChildren[i].dataset.pokeNumber == e.target.parentNode.dataset.pokemonNumber) {
  //         let unfavoriteItem = e.target.parentNode.dataset.pokemonNumber
  //         let matchingItem = document.querySelector(`[data-pokemon-number~="${unfavoriteItem}"]`)
  //         matchingItem.className = 'hidden'
...
const removeFavoritesRender = (e, pokemonCard) => {
  ...
  let matchingItem = document.querySelector(
    `[data-pokemon-number~="${unfavoriteItem}"]`
  );
